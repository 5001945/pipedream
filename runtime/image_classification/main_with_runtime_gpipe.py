...
sys.path.append("..")
import runtime_gpipe as runtime
import sgd

from typing import TYPE_CHECKING
if TYPE_CHECKING:
    from .. import runtime_gpipe as runtime
...
def run_forward(r: runtime.MapaeanGPipe):
    r.run_forward_4times()

def run_backward(r: runtime.MapaeanGPipe, optim):
    optim.zero_grad()
    r.run_backward_4times()
    optim.step()


def train(train_loader, r: runtime.MapaeanGPipe, optimizer, epoch):
    ...
    if args.no_input_pipelining:  # GPipe에서는 항상 True이다.
    ...
    # for i in range(num_warmup_minibatches):  # GPipe에서는 무시 가능
    #     r.run_forward()
