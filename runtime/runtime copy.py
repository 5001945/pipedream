...
class MapaeanGPipe():
    ...
    def __init__(self,
        ...
        *,
        ...
        module_list = model[:-1]
        criterion = model[-1]
        self._balance = list(map(round, balance))
        self.microbatches = microbatches

        devices = get_cuda_devices()
        assert len(devices) == len(self._balance)
        assert sum(self._balance) == len(module_list)

        # Build pipeline and set the module structure in each devices.
        self.pipeline: list[tuple[int, torch.device, list[nn.Module]]] = []
        mi = 0  # module_index
        for i, device in enumerate(devices):
            mn = self._balance[i]  # module_number
            modules_i = nn.Sequential(*module_list[mi:mi+mn])
            self.pipeline.append((i, device, modules_i.to(device)))
            mi += mn

        self.communication_handler = communication.CommunicationHandler(
        ...
        self.communication_handler.initialize(
            receive_ranks,
            send_ranks,
